...
const Dashboard: React.FC = () => {
  ...
  const renderCard = (day: any, index: number, isCustom: boolean) => {
      ...
      return (
        ...
        >
          {/* Card Container */}
          <div className={`h-full p-6 bg-card border ${isDraftActive ? 'border-primary shadow-glow' : 'border-zinc-800 hover:border-zinc-600'} flex flex-col justify-between relative overflow-hidden`}>
            
            {/* Background Number */}
            <div className="absolute -right-2 -top-4 text-[80px] font-black text-white/0 stroke-text select-none opacity-[0.15] pointer-events-none font-heading">
              {isCustom ? 'C' : `0${index + 1}`}
            </div>

            <div className="relative z-10">
              <div className="flex justify-between items-start mb-4">
                <span className={`text-[9px] font-mono uppercase tracking-widest border px-2 py-1 bg-black ${isCustom ? 'text-emerald-500 border-emerald-900' : 'text-zinc-500 border-zinc-800'}`}>
                  {isCustom ? 'CUSTOM WORKOUT' : `Protocol 0${index + 1}`}
                </span>
                {!isDraftActive && <ChevronRight className="text-zinc-700 group-hover:text-primary transition-colors" size={16} />}
              </div>

              <h3 className={`text-lg font-black uppercase tracking-wide mb-6 ${isDraftActive ? 'text-primary' : 'text-white'}`}>
  ...
  return (
    <div className="animate-fade-in pb-10">
      ...
      {draftSession && (
        <div className="mb-8 relative overflow-hidden group border border-primary/50 bg-primaryDim/5">
          ...
          <div className="relative p-6 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <button 
              ...
              className="w-full md:w-auto flex items-center justify-center space-x-3 bg-primary text-black px-8 py-4 font-black text-xs uppercase tracking-widest hover:bg-primaryHover transition-colors shadow-glow"
            >
              <span>RESUME</span>
      ...
      {/* TABS SWITCHER */}
      <div className="flex p-1 bg-zinc-900/50 border border-zinc-800 mb-8 backdrop-blur-sm">
          <button 
        ...
        {activeTab === 'standard' ? (
             ...
             <>
                {customPrograms.length === 0 ? (
                    <div className="text-center py-16 border-2 border-dashed border-zinc-800 bg-zinc-900/20">
                        ...
                        <button 
                            ...
                            className="bg-zinc-800 text-white border border-zinc-700 px-6 py-3 text-xs font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-black hover:border-emerald-500 transition-all"
                        >
                            + Creează Acum
                        </button>
                    </div>
                    ...
                    <>
                        ...
                        <button 
                            onClick={() => navigate('/settings/program-editor')}
                            className="w-full mt-4 py-4 border border-zinc-800 bg-black text-zinc-500 hover:text-white hover:border-zinc-600 transition-all text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 group"
                        >
